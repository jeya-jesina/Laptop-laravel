...
export default function EditBudget() {
  ...
  const fetchBudget = async () => {
    ...
    try {
      ...
      if (res.data.status) {
        ...
        setName(b.name || "");
        setMinPrice(b.min_price !== null && b.min_price !== undefined && b.min_price !== "" ? String(b.min_price) : "");
        setMaxPrice(b.max_price !== null && b.max_price !== undefined && b.max_price !== "" ? String(b.max_price) : "");

      } else {

        show("error", "Not found", res.data.message || "Budget not found.");
      ...
      console.error(err);
      show("error", "Server error", "Unable to load the budget.");
  ...
  const handleSubmit = async () => {
    ...
    if (!name.trim()) {
      show("warn", "Missing field", "Budget name is required.");
      return;
    }
    ...
    try {
      ...
      if (res.data.status) {
        show("success", "Budget updated!", `"${name.trim()}" saved successfully.`);
      ...
      <div className="sbf-page">
        ...
        <div className="sbf-card">
          ...
          <div className="sbf-header">
            <div className="sbf-icon-box">💰</div>
            <div className="sbf-header-text">
              <h1>Edit Budget</h1>
              <p>Update price range budget</p>
            </div>
          </div>
          ...
          <div className="sbf-body">
            ...
            {fetching ? (
              ...
              <>
                ...
                <div className="sbf-field">
                  <div className="sbf-label-row">
                    <span className="sbf-label">Budget Name</span>
                    <span className="sbf-char">{name.length}/50</span>
                  </div>
                  <input
                    type="text"
                    className="sbf-input"
                    placeholder="e.g. Under ₹15,000"
                    value={name}
                    maxLength={50}
                ...
                <div className="sbf-row">
                  <div className="sbf-field">
                    <div className="sbf-label-row">
                      <span className="sbf-label">Min Price (₹)</span>
                    </div>
                    <input
                      type="number"
                      className="sbf-input"
                  ...
                  <div className="sbf-field">
                    <div className="sbf-label-row">
                      <span className="sbf-label">Max Price (₹)</span>
                    </div>
                    <input
                      type="number"
                      className="sbf-input"
                ...
                <button className="sbf-btn-cancel" onClick={() => navigate("/budget")}>
                  Cancel
                </button>
              </>
            )}
