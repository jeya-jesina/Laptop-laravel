...
export default function EditProfession() {
  ...
  const fetchProfession = async () => {
    ...
    try {
      ...
      if (res.data.status) {
        ...
        setSelectedCompany(p.company_id ? String(p.company_id) : "");
        localStorage.setItem("selected_company_id", p.company_id || "");
        setName(p.name || "");

      } else {

        show("error", "Not found", res.data.message || "Profession not found.");
      ...
      console.error(err);
      show("error", "Server error", "Unable to load the profession.");
  ...
  const handleSubmit = async () => {
    ...
    if (!name.trim()) {
      show("warn", "Missing field", "Profession name is required.");
      return;
    }

    setLoading(true);
    try {
      const res = await api.post("/profession/update", {
        id,
        name: name.trim(),
      });
      if (res.data.status) {
        show("success", "Profession updated!", `"${name.trim()}" saved successfully.`);
        ...
        <div className="sbf-card">
          ...
          <div className="sbf-header">
            ...
            <div className="sbf-header-text">
              ...
              <p>Update laptop usage category</p>
            </div>
          </div>
          ...
          <div className="sbf-body">
            ...
            {fetching ? (
              ...
              <>
                ...
                <div className="sbf-field">
                  <div className="sbf-label-row">
                    <span className="sbf-label">Profession Name</span>
                    <span className="sbf-char">{name.length}/50</span>
                  </div>
                  <input
                    type="text"
                    className="sbf-input"
                    placeholder="e.g. Student, Gaming, Office Work..."
                    value={name}
                    maxLength={50}
                ...
                <button className="sbf-btn-cancel" onClick={() => navigate("/profession")}>
                  Cancel
                </button>
              </>
            )}
